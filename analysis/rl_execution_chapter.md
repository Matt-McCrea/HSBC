...
the episode budget available in a simulator-based study (§3.8).
...
**The simulated market over-executes by roughly threefold.** Execution rate is 17–18% in
the generative arm. Measured over 54 replay episodes on the real INTC stream — executions
per new order, the identical quantity computed by the identical code — it is **6%**
(median 5%). A generated order is therefore about three times as likely to execute as a
real one, so measured shortfall is optimistic relative to live trading: fills that a real
book would not have provided are provided here.

The real rate varies materially by day and time of day (single episodes range 5–9%),
which is why the figure quoted is the 54-episode mean rather than any one measurement.
It sits close to the 4–6% independently reported for this symbol, which is reassuring for
the definition. One caveat on that definition: LOBSTER event type 4 marks each visible
execution, so an order filling in several parts contributes more than once, making the
real rate a mild upper bound on the fraction of orders that execute at all — and the
over-execution multiple correspondingly a mild lower bound.
...
## 3.3 Simulator realism, measured against real order flow

The **replay arm** answers the question the chapter exists for. It runs identical
policies, on identical held-out seeds, through the identical estimator — the only change
is that the background market is the *real* INTC message stream from $t_0$ rather than one
generated by the world agent. Because no diffusion sampling is involved, episodes cost
5.2 s against the generative arm's 823 s, so this arm is cheap to run at scale.

Both columns below are 54 episodes, measured by the same code:

| Quantity | Generative | **Real (replay)** | Reading |
|---|---|---|---|
| Drift over episode | +5.33 bps, $t$=4.30 | **+1.62 bps, $t$=0.81** | martingale holds on real data, **fails in the simulator** |
| Execution rate | 0.17 | **0.06** | simulator fills ~3$\times$ too readily |
...
mid-price. On real data that assumption survives ($t$=0.81); in the simulator it does not
($t$=4.30, and $t$=7.88 on the training log). The simulator's drift is not a small
imperfection — it is large enough to reward a seller for delaying, which is precisely the
behaviour the risk term exists to discourage, so it acts directly against the objective.

**Three biases point the same way.** The generative market drifts upward, fills too
readily and moves its price too much, and each of those flatters a single-sided
liquidation. Volatility, by contrast, is well matched (166 vs 155.6), so this is not a
simulator that is simply too noisy — it is one that is too *accommodating*.

### Cost of aggression, and the shape of impact

Splitting slippage by execution style (§3.6) in both arms:

| Style | Generative | **Real (replay)** |
|---|---|---|
| Passive limit | −2.031 bps | **−0.826 bps** |
| Crossing limit | +2.671 bps | **+1.388 bps** |
| Market order | +3.369 bps | **+1.411 bps** |
| Passive→aggressive swing | 5.40 bps | **2.24 bps** |

The simulator roughly doubles the cost of crossing the spread. The qualitative conclusion
of §3.6 survives — aggression still dominates size in both — but its magnitude is a
simulator artefact and should be quoted from the replay column.

**On real data the square-root law is identifiable.** Against real flow the square-root
fit beats the linear one by $R^2$ 0.2219 vs 0.1794, a gap of 0.043. In the generative arm
the same comparison gave gaps of 0.003–0.005 with the winner flipping between samples.
So the real market shows the concave impact the literature reports, and the simulated one
does not reproduce it — a specific, falsifiable deficiency rather than a vague one.

### What this means for using the simulator

The simulator is usable for *relative* policy comparison and unusable for *absolute* cost
estimation. Rankings are largely preserved — TWAP leads in both arms, permanent impact is
absent in both, aggression dominates size in both — but every level is optimistic, and the
drift actively distorts a single-sided objective. Any shortfall figure taken from it
should be treated as a lower bound on live cost.
