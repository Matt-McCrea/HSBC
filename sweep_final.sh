...
phase_predictive() {
  ...
  # actually promises to compute. The genuine TRADES-configuration comparison is
  # the nofix batch below, scored separately with its own --real reference.
  ...
  # --------------------------------------------------------------------
  # The genuine TRADES-configuration comparison: the no-fixes checkpoint
  # (nofix_baseline.sh) at DDIM-1 and DDPM-100, same checkpoint, same day.
  # Generated by nofix_baseline.sh's own stage_sims (control_ddim1,
  # control_ddpm100, added 2026-08-15) -- this phase only SCORES, it never
  # runs a simulation itself, since that needs the pipeline flags OFF and
  # this script's preflight forces them ON for CK_BASE/CK_FINAL. If those
  # runs have not happened yet, this block just reports NOT FOUND and
  # moves on; it does not block the rest of the phase.
  note "=== nofix TRADES-configuration comparison ==="
  local REAL_NOFIX="$L/market_replay_INTC_2015-01-30_10-00-00_30/processed_orders.csv"
  if [ ! -f "$REAL_NOFIX" ]; then
    note "SKIP nofix predictive: no real reference at $REAL_NOFIX"
  else
    # Independent resolver, not the shared add()/args above -- this pair must
    # be scored against REAL_NOFIX, never accidentally folded into the main
    # four-way batch or its $REAL.
    local nofix_args=()
    add_nofix() {  # add_nofix <label> <glob>
      local p; p=$(ls -d $2 2>/dev/null | head -1)
      if [ -n "$p" ] && [ -f "$p/processed_orders.csv" ]; then
        nofix_args+=(--gen "$1=$p/processed_orders.csv"); note "  predictive: $1 <- $p"
      else
        note "  predictive: $1 NOT FOUND ($2)"
      fi
    }
    # Prefer whatever checkpoint nofix_baseline.sh actually picked (its own
    # CHECKPOINT.txt, most recent invocation), falling back to the val_ema
    # this was written against (2026-08-15: epoch 2, val_ema=0.667) in case
    # that file is not present on this machine.
    local NOFIX_CKTAG
    NOFIX_CKTAG=$(ls -t nofix_*/CHECKPOINT.txt 2>/dev/null | head -1 | xargs cat 2>/dev/null \
                  | xargs -I{} basename {} .ckpt | sed 's/_INTC.*//; s/_epoch.*//; s/val_ema=//')
    NOFIX_CKTAG="${NOFIX_CKTAG:-0.667}"
    note "  nofix checkpoint tag: $NOFIX_CKTAG"
    add_nofix nofix_ddim1   "$L/world_agent_INTC_2015-01-30_10-00-00_30_DDIM_0.0_1_val_ema=${NOFIX_CKTAG}*"
    add_nofix nofix_ddpm100 "$L/world_agent_INTC_2015-01-30_10-00-00_30_DDPM_0.0_100_val_ema=${NOFIX_CKTAG}*"

    if [ ${#nofix_args[@]} -eq 4 ]; then
      python -u -m evaluation.quantitative_eval.predictive_batch \
        --real "$REAL_NOFIX" "${nofix_args[@]}" --seeds 3 \
        --out-dir "$OUT/predictive_nofix" \
        2>&1 | tee "$OUT/predictive_nofix.log"
      note "nofix predictive done -> $OUT/predictive_nofix"
    else
      note "SKIP nofix predictive: control_ddim1 and/or control_ddpm100 not found"
      note "  run: ./nofix_baseline.sh sims   (generates both, on the current checkpoint)"
    fi
  fi
}
