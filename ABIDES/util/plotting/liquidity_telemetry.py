...
def bin_and_sum(s, binwidth):
    ...
    """
    if len(s) == 0:
        return pd.Series(dtype=float, index=pd.IntervalIndex([]))
...
def np_bar_plot_hist_input(counted):
    ...
    idx = counted.index
    if hasattr(idx, "categories"):        # CategoricalIndex of intervals
        intervals = idx.categories
    elif hasattr(idx, "left"):            # IntervalIndex directly
        intervals = idx
    else:
        # unexpected (e.g. RangeIndex) — nothing plottable
        return np.array([]), np.array([]), np.array([])

    bins = list(intervals.left) + [intervals.right[-1]]
    ...
    width = np.array([x.total_seconds() / 86400 for x in width])
